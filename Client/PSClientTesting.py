...
class PSClient(HelperClient):
	...
	def publish(self,topic,payload, qos=1, no_response=0, **kwargs):
		...
		response = self.put(topic, payload,callback = self.responseCallback, qos = qos, no_response = no_response,**kwargs)
		timeStamp=time.time()
		#print(response.pretty_print())
	...
	def responseCallback(self,response):
		timeStamp = time.time()
		request = self.pendingRequests[response.token]
		#TO SAVE PUBACK RECEPTION (for testing)
		if request.code == 3:
		...
		if response is not None:
			...
			try:
				...
				#lines to print in the file (ONLY FOR SIMULATIONS)
	...
	def subscribe(self, topic, qos=1, **kwargs):
		...
		f.close()
		response = self.observe(topic,self.responseCallback, qos=qos, **kwargs)
		return
