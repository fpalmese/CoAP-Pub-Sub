...
class Client:
  ##added by me create gwifo for sending
    ...
    def connect(self,host,port=1883,keepalive=60):
        ...
        connect.Duration = self.keepalive
        ...
        if qos in [-1,0]:
            ...
            while(True):
                ...
                try:
                    ...
                    if self.waitfor(MQTTSN.PUBACK)==None:
                        print("didnt receive PUBACK")
                        #publish.Flags.DUP = 1
                        sys.stdout.flush()
                        continue
                    else:
                        ...
                        f = open("csv/"+self.clientid+".csv","a")
        ...
        #qos==2
            ...
            while(True):
                ...
                try:
                    ...
                    self.sock.send(a.encode())
                    self.__receiver.outMsgs[publish.MsgId] = publish
                    self.lookfor(MQTTSN.PUBREC)                    
                    if self.waitfor(MQTTSN.PUBREC) is None:
                        print("didnt receive PUBREC")
                        sys.stdout.flush()
                    else:
                        print("received PUBREC, sending PUBREL")
                        pubrel = self.__receiver.pubrel
                        pubrelPack = pubrel.pack()
                        ntimes = 0
                        while(True):
                            try:
                                ntimes +=1
                                if ntimes>7:
                                    break
                                pubrelPack = pubrel.pack()
                                self.sock.send(pubrelPack.encode())
                                self.__receiver.outMsgs[pubrel.MsgId] = pubrel
                                self.lookfor(MQTTSN.PUBCOMP)
                                if self.waitfor(MQTTSN.PUBCOMP) is None:
                                    print("didnt receive PUBCOMP")
                                    sys.stdout.flush()
                                else:
                                    timeStamp = time.time()
       	...
       	#if qos not in [-1,0]:
        #    thread=threading.Thread(target=self.getPubAck,args=(topic,payload,qos,retained,ntimes))
        #    thread.start()
            
        #return(self.waitfor(MQTTSN.PUBACK))
        return publish.MsgId

    def getPubAck(self,topic,payload,qos,retained,ntimes):
        self.lookfor(MQTTSN.PUBACK)
        if self.waitfor(MQTTSN.PUBACK)==None:
            ntimes +=1
            print("didn't receive puback")
            if ntimes>1:
                return
            print("ntimes",ntimes)
            self.publish(topic,payload,qos,retained,ntimes)
        else:
            timeStamp = time.time()
            f = open("csv/pub.csv","a")
            f.write(payload+";"+str(timeStamp)+"\n")
            f.close()
        return
...
def publish(topic, payload, retained=False, port=1883, host="localhost",sock = None):
  publish = MQTTSN.Publishes()
  publish.Flags.QoS = 3
  publish.Flags.Retain = retained  
  ...
  if type(topic) is str:
    ...
    publish.Flags.TopicIdType = MQTTSN.TOPIC_PREDEFINED
    publish.TopicId = topic
            #########
